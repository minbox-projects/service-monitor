...
## 功能特性
- **智能告警与去重**：采用后台队列及随机 ID 后缀（防止反垃圾屏蔽）处理告警发送，支持全局限流（防止邮件轰炸）。
...
- **国际化 (i18n)**：所有用户可见文本（邮件标题、告警内容、报告、恢复通知）支持中英文切换，通过 `config.yaml` 中的 `language` 字段配置。
...
3. **配置文件**
   ...
   | :--- | :--- | :--- | :--- |
   | `language` | 界面语言 (`"zh"` 中文 / `"en"` 英文)，影响邮件、告警、报告中的所有文本 | `"en"` | - |
   | **`logging`** | **日志存储配置** | | |
